      ...
      <header className="border-b border-[#2d2d4a] bg-[#1a1a2e]/50 backdrop-blur-xl sticky top-0 z-50">
        <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => router.push('/settings')} className="p-2 hover:bg-[#252540] rounded-xl transition-colors text-[#8b8ba7]">
              <Settings className="w-5 h-5" />
            </button>
            <button onClick={clearToken} className="p-2 hover:bg-[#252540] rounded-xl transition-colors text-[#8b8ba7]">
      ...
      <main className="max-w-6xl mx-auto px-4 py-8">
        ...
        {status && (
          <>
            ...
            <ActivityFeed history={status.history} timezone={status.settings.timezone} />
          </>
        )}
      </main>
    </div>
